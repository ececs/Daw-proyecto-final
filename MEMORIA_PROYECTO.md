...
**Confirmación de acciones en frontend:** La implementación de `interrupt/resume` persistente en LangGraph requiere una gestión de checkpoints y reanudación de grafos que supone una complejidad de infraestructura significativa. Para mantener el alcance del proyecto dentro del tiempo disponible, se optó por una confirmación ligera en el lado cliente: el agente solicita confirmación mediante texto estructurado y el frontend la procesa antes de ejecutar la herramienta. Este enfoque cubre el requisito funcional sin comprometer la estabilidad del sistema.
